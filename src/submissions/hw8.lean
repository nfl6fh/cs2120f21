...
Yes, if (x → y) → (y → z) → (x → z), and (x → y) → (y → x), then
we can substitute x in for the z in the transitive definition so 
that becomes (x → y) → (y → x) → (x → x).
-/
...
/-
They preserve intersection, subset, and union. However,
they do not preserve equality.
-/
...
/-
A well order relation is a type of a total order
relation, where every non-empty subset has a 'least' element.
-/
