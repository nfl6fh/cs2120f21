...
-- given a proof of P and a proof of Q, 
  we can give a proof of P and Q
...
-- If you select a item t of type T at random and it
  has property Q, then all objects of type T will
  also have property Q since you could've selected
  any object of type T
...
-- We have the proof qf that all items t of type T have
the property Q. We can then apply this to item t and say 
that t has property Q using the for all elimination rule.
...
-/
example : ∀ (p : Person), KnowsLogic Lynn → BetterComputerScientist Lynn:=
begin
  assume p LynnKnowsLogic,
...
/-
show that P implies false by assuming P and
proving P → false → true, which will proove ¬P
-/
...
To prove P, assume ¬P and show that P implies false.
From this derivation you can conclude not P.
...
is not valid, and that accepting the axiom
of the ¬P suffices to establish negation
by contradiction (better called double not implication)
as a theorem.
-/
...
/-
For all p of type Person, if p is Nice and
Talented, then for all q of type Person, q
likes p.
or more simply:
if Person p is Nice and Talented, then everyone
likes them
-/
...
-- how many cases will need to be considered? 2
-- list the cases (informaly)
    -- the case in which the car is heavy
    -- and the case in which the car is light
