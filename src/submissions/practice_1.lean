...
/-
We can assume that if given a random object x, if it has property P,
then all objects of the same type as x will have property P.
-/
...
begin
  let pf_raining,
  exact if_raining_then_streets_wet,
end 
...
the and elimination rule to a proof of (P ∧ Q) and a proof of R.
What remains, then, is to obtain these proofs.
But this is easily done by the application of
the and intro rule to p, q, and r. QED. 
-/
